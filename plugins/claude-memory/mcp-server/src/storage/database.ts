...
export class MemoryDatabase {
  ...
  listMemories(filter: MemoryFilter = {}): Memory[] {
    ...
    if (filter.tags && filter.tags.length > 0) {
      ...
      const escapedTags = filter.tags.map(tag =>
        tag.replace(/\\/g, '\\\\').replace(/%/g, '\\%').replace(/_/g, '\\_')
      );
  ...
  getStats(): {
    ...
    const memories = this.db.prepare('SELECT COUNT(*) as count FROM memories WHERE archived = 0').get() as any;
    const conversations = this.db.prepare('SELECT COUNT(*) as count FROM conversations WHERE archived = 0').get() as any;
    const knowledge = this.db.prepare('SELECT COUNT(*) as count FROM knowledge WHERE archived = 0').get() as any;
    ...
    const totalFileSize = this.db.prepare('SELECT SUM(file_size) as total FROM memories WHERE archived = 0').get() as any;
