...
export class MemoryTools {
    ...
    async remember(args) {
        const memory = await this.storage.createMemory(args.type || 'note', args.title, args.content, args.tags || [], args.metadata);
    ...
    async recall(args) {
        ...
        if (args.useSemanticSearch) {
            ...
            const formatted = results.map((r, i) => `${i + 1}. **${r.memory.title}** (Similarity: ${(r.similarity * 100).toFixed(1)}%)\n` +
            ...
            const formatted = searchResults.map((r, i) => `${i + 1}. **${r.memory.title}** (Score: ${r.score.toFixed(2)})\n` +
                `   Type: ${r.memory.type} | Tags: ${r.memory.tags.join(', ') || 'none'}\n` +
                `   ${r.memory.content.slice(0, 200)}${r.memory.content.length > 200 ? '...' : ''}\n` +
                `   ID: ${r.memory.id}\n`).join('\n');
            return {
    ...
    async listMemories(args) {
        ...
        const formatted = memories.map((m, i) => `${i + 1}. **${m.title}**\n` +
    ...
    async saveConversation(args) {
        ...
        const conversation = await this.storage.saveConversation(args.title, messages, args.metadata);
        return {
            content: [
                {
                    type: 'text',
    ...
    async loadConversation(args) {
        ...
        const formatted = conversation.messages.map(msg => `**${msg.role.toUpperCase()}** (${new Date(msg.timestamp).toLocaleString()}):\n${msg.content}\n`).join('\n---\n\n');
        return {
            content: [
                {
                    type: 'text',
    ...
    async saveSession(args) {
        const session = await this.storage.saveSession(args.title, args.context, args.files, args.variables, args.metadata);
        return {
            content: [
                {
                    type: 'text',
